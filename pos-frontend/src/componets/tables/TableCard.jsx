...
const TableCard = ({name, status, initial}) => {
  ...
  return (
    <div onClick={handleClick} className="w-[300px] bg-[#262626] hover:bg-[#2e2e2e] rounded-lg p-4 mb-4 cursor-pointer">
      <div className="flex items-center justify-between">
        <h1 className="text-[#f5f5f5] text-xl font-semibold">{name}</h1>
        <p className={` ${status === "Заброньовано" ? "text-green-600 bg-[#2e4a40]" : "bg-[#f6b100] text-white"} px-2 py-1 rounded-lg`}>
